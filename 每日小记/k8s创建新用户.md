...
第一步：执行bash k8s-add-service-account-config.sh lf staging

```sh
## k8s-add-service-account-config.sh
#!/bin/bash
set -e
set -o pipefail
...
第二步：执行kubectl apply -f permission.yaml
...
```sh
KUBECONFIG=/tmp/kube/k8s-lf-staging-conf kubectl get pods -n staging
```

第四步：移动conf

```sh
mv /tmp/kube/k8s-lf-staging-conf ~/.kube/config
```
...
参考：
https://blog.csdn.net/sysushui/article/details/104881715
